...
uint64_t unwrap(WrappingInt32 n, WrappingInt32 isn, uint64_t checkpoint) {
    ...
    if (out_candidate > checkpoint && ((out_candidate - checkpoint) > (UINT32_MAX / 2)) && (out_candidate_higher_32 != 0)) {
        out_candidate -= (1ul << 32);
    } else if (out_candidate < checkpoint && ((checkpoint - out_candidate) > (UINT32_MAX / 2)) && (out_candidate_higher_32 != (UINT64_MAX - UINT32_MAX))) {
        out_candidate += (1ul << 32);
    }
